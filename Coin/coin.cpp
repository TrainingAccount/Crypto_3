...
int main()
{
    ...
    else
    {
        ...
        cout << "File 'salt.txt' was generated, please restart the application" << endl;
        system("pause");
        fclose(f);
        return 0;
    }
    ...
    {
          char temp[40];
          strcpy(temp, md5(buf).c_str());
          fprintf(f, "Hash: %s\nNumber: %d\nSalt: %s", temp, number, salt);
          cout << "Your random number and hash saved in 'result.txt' file" << endl;
          system("pause");
          return 1;
    }
    else
    {
        cout << "Can't open or create 'result.txt' file" << endl;
        system("pause");
        return -1;
    }
}
...
string md5(string h)
{
    ...
    //for (int i = 0 ; i < maxLength; i+=4)
      //  X[i/4] = flow[i] * suff1 + flow[i+1] * suff2 + flow[i+2] * 256 + flow[i+3];
